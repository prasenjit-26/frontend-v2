...
export const getIsEchoooEth = () =>
  Boolean(
    // @ts-ignore
    window.echoooEth &&
      // @ts-ignore
      !allNonMetamaskFlags.some(flag => window.echoooEth?.[flag])
  );

export function getInjectedProvider() {
  // @ts-ignore
  const ethereum: any = window.echoooEth;
  console.log('ethereum', ethereum);
  const provider = ethereum || (window as any).web3?.currentProvider;
...
export class EchoooEthConnector extends Connector {
    ...
    if (provider) {
      ...
      try {
        ...
        if ((err as WalletError).code === 4001) {
          ...
          console.log('Please connect to FoxWallet.');
        } else {
          console.error(err);
        }
      }
...
export const EchoooEthConnectorType = typeof EchoooEthConnector;
